...
from agent_inspect.tools.utils import map_subgoal_validations_to_binary_matrix
...
class StatisticAnalysis:
    ...
    def calculate_probabilities(binary_matrix: list[list[int]]) -> list[float]:
        ...
        :param binary_matrix: 2D list where inner lists cannot have different lengths. Each inner list represents judge scores for a subgoal.
        :return: List of probabilities (means) for each subgoal.
        :raises InvalidInputValueError: If binary matrix is empty or inner lists have different lengths.
    ...
    @staticmethod
    def compute_statistic_analysis_result(
        data_sample: SubgoalErrorAnalysisDataSample,
    ) -> StatisticAnalysisResult:
        """Returns the judge expectation and variance on a single data sample.

        :param data_sample: a :obj:`~agent_inspect.models.tools.analysis_models.SubgoalErrorAnalysisDataSample` object containing subgoal validations with judge trial explanations.
        ...
        :raises InvalidInputValueError: If subgoal validations are empty or missing judge explanations.
        ...
        for subgoal_validation in data_sample.subgoal_validations:
            if len(subgoal_validation.explanations) < 2:
                raise InvalidInputValueError(
                    internal_code=ErrorCode.MISSING_VALUE.value,
                    component_code=ToolComponent.TOOL_ERROR_CODE.value,
                    message="Each SubGoalValidationResult must contain at least one judge explanation besides the summarized one.",
                )
            one_subgoal_judge_score = map_subgoal_validations_to_binary_matrix(
                subgoal_validation.explanations[1::]
            )  # Skip the first explanation which is the summarized one
        ...
        overall_expectation = np.mean(
            subgoal_means
        )  # Mean across all subgoals subgoal len of outer array(binary matrix)
