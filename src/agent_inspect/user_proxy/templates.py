...
If the conversation has concluded, prepare to respond with either the stop sequence {stop_sequence_done}, {stop_sequence_delegate}, or {stop_sequence_blocked} in the next response generation phase.
Otherwise, DO NOT consider termination if the current conversation is not at its final stage.
"""
...
1. {stop_sequence_done}
2. {stop_sequence_delegate}
3. {stop_sequence_blocked}
...
2. {check_delegate}:
   {stop_sequence_delegate}

3. {check_blocked}:
   {stop_sequence_blocked}
