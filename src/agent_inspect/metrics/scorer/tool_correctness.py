...
from agent_inspect.metrics.validator.tool_call_completion import (
...
class ToolCorrectnessMetric(LLMBasedMetric):
    """
    Metric to calculate the correctness rate of tool calls made by an agent in its entire dialogue trace. This metric only supports finding the matched tool call in one single turn. The final score is computed as the ratio of correct tool calls to the total number of tool calls made.
    ...
    def evaluate(
        ...
        """
        tool_call_validator = ToolCallCompletionValidator(
        ...
        if not evaluation_data_sample.expected_tool_calls:
            raise InvalidInputValueError(
                internal_code=ErrorCode.EMPTY_VALIDATION_RESULT.value,
                message="No expected tool calls present to evaluate for tool correctness score.",
            )
            ...
            for expected_tool_call in evaluation_data_sample.expected_tool_calls:
                if expected_tool_call.turn == idx:
                    validation_result = asyncio.run(
                        tool_call_validator.validate_static_last_turn(
                            agent_trace_turns=turns_grouping,
                    ...
                    explanations.append({expected_tool_call.tool: validation_result.explanations})
    ...
    def get_tool_correctness_score_from_validation_results(
        ...
        if not validation_results:
            raise InvalidInputValueError(
                internal_code=ErrorCode.EMPTY_VALIDATION_RESULT.value,
                message="No validation results present to aggregate for tool correctness score.",
            )
