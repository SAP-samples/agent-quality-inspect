...
from agent_inspect.metrics.validator.templates import TOOL_CORRECTNESS_TEMPLATE
from agent_inspect.metrics.validator.constants import (
    ...
    ARGUMENT_PASSED_LLMJ_EXPLANATION,
    ARGUMENT_FAILED_LLMJ_EXPLANATION,
    OUTPUT_PASSED_EXACT_MATCH_EXPLANATION,
    OUTPUT_FAILED_EXACT_MATCH_EXPLANATION,
    OUTPUT_PASSED_LLMJ_EXPLANATION,
...
from agent_inspect.metrics.utils.expected_tool_validators import (
...
class ToolCallCompletionValidator(Validator):
    ...
    async def validate_static_last_turn(
        self, agent_trace_turns: List[TurnTrace], expected_tool_call: ExpectedToolCall
        ...
        """
        TraceValidator.validate_turn_traces_content(agent_trace_turns)
        TraceValidator.validate_agent_trace_if_empty(agent_trace_turns)
        ...
        return await self._validate_per_turn(
            agent_turn_trace=agent_turn_trace,
            expected_tool_call=expected_tool_call,
            num_judge_trials=num_judge_trials,
            include_judge_explanation=include_judge_explanation,
        )

    async def validate_multi_turns(
        self, agent_trace_turns: List[TurnTrace], expected_tool_call: ExpectedToolCall
    ) -> ToolCallValidationResult:
        """
        Returns a :obj:`~agent_inspect.models.metrics.validation_result.ToolCallValidationResult` indicating whether the tool call is correctly completed by the agent in any turn of the agent trace.
        Iterates through all user provided turn traces from first to last, stopping as soon as a valid tool call is found or when the last turn is reached.

        :param agent_trace_turns: A list of :obj:`~agent_inspect.models.metrics.agent_trace.TurnTrace` representing the entire agent trace.
        :param expected_tool_call: An :obj:`~agent_inspect.models.metrics.agent_data_sample.ExpectedToolCall` representing the expected tool call checklist to validate against.
        :return: a :obj:`~agent_inspect.models.metrics.validation_result.ToolCallValidationResult` indicating whether the tool call is correctly completed by the agent.
        ...
        result = None
        for agent_turn_trace in agent_trace_turns:
            result = await self._validate_per_turn(
                agent_turn_trace=agent_turn_trace,
                expected_tool_call=expected_tool_call,
                num_judge_trials=num_judge_trials,
                include_judge_explanation=include_judge_explanation,
            )
            # If tool call is correctly completed, return immediately
            if result.is_completed:
                return result

        # If no valid tool call found in any turn, return the last result, which contains explanations ONLY for the last attempted validation, but will discard all the previous validation results.
        # If no turns were processed, result will be None, but this shouldn't happen after TraceValidator.validate_agent_trace_if_empty
        if result is None:
            raise EvaluationError(
                internal_code=ErrorCode.INVALID_VALUE.value,
                message="No turns were processed during validation",
            )
        return result

    async def _validate_per_turn(
        self,
        agent_turn_trace: TurnTrace,
        expected_tool_call: ExpectedToolCall,
        num_judge_trials: int,
        include_judge_explanation: bool,
    ) -> ToolCallValidationResult:
        """
        Helper method to validate a single turn of the agent trace.

        :param agent_turn_trace: A single :obj:`~agent_inspect.models.metrics.agent_trace.TurnTrace` to validate.
        :param expected_tool_call: An :obj:`~agent_inspect.models.metrics.agent_data_sample.ExpectedToolCall` representing the expected tool call checklist to validate against.
        :param num_judge_trials: The number of LLM-as-a-judge runs for validation.
        :param include_judge_explanation: Whether to include judge explanations in the output.
        :return: a :obj:`~agent_inspect.models.metrics.validation_result.ToolCallValidationResult` indicating whether the tool call is correctly completed in this turn.
        """
        ...
        res_tool_call_validation = None
        for agent_single_tool_step in agent_tool_steps:
            # Check steps:
            # Step 1. check tool name match
            if (
                agent_single_tool_step.tool is not None
                and expected_tool_call.tool.strip() == agent_single_tool_step.tool.strip()
            ):
                ...
                all_parameters_correct = (
                    all(res["is_completed"] for res in parameter_validation_results.values())
                    and is_tool_output_correct
                )
        ...
        # no matching tool name found for this expected tool call in the current turn, return failed validation result with explanation
        if res_tool_call_validation is None:
            return ToolCallValidationResult(
                is_completed=False,
                expected_tool_call=expected_tool_call,
                explanations=[TOOL_NOT_FOUND_EXPLANATION.format(tool_name=expected_tool_call.tool)],
            )
        return res_tool_call_validation
    ...
    def get_tool_steps_from_agent_trace(agent_turn_trace: TurnTrace) -> List[Step]:
        ...
        """
        tool_steps = []
        if agent_turn_trace.steps is not None:
    ...
    async def _validate_single_parameter(
        ...
        if param.check is not None:
            (
                is_correct,
                explanation_list,
            ) = await self.validate_tool_call_parameter_by_llmj(
                llm_client=self.llm_client,
                param_name=param.name,
                param_check=param.check,
    ...
    async def _validate_output(
        ...
        if expected_tool_call.expected_output is not None:
            if (
                expected_tool_call.expected_output.value is not None
                and expected_tool_call.expected_output.check is not None
            ):
                raise InvalidInputValueError(
                    internal_code=ErrorCode.INVALID_VALUE.value,
                    message="ExpectedToolCall output cannot have both value and check specified at the same time.",
                )
            if expected_tool_call.expected_output.check is not None:
                (
                    is_tool_output_correct,
                    output_explanation,
                ) = await self.validate_tool_call_output_by_llmj(
                    llm_client=self.llm_client,
                    output_check=expected_tool_call.expected_output.check,
                    actual_tool_call=agent_single_tool_step,
                    num_judge_trials=num_judge_trials,
                    include_judge_explanation=include_judge_explanation,
                )
                ...
                is_tool_output_correct, output_explanation = (
                    self.validate_tool_call_output_by_exact_match(
                        expected_output_value=expected_tool_call.expected_output.value,
                        actual_tool_call_step=agent_single_tool_step,
                    )
                )
    ...
    def _aggregate_success(
        ...
        for p in parameter_validation_results.values():
            correct_explanation.extend(
                p["explanation"] if isinstance(p["explanation"], list) else [p["explanation"]]
            )
        if output_explanation is not None:
            correct_explanation.extend(
                output_explanation if isinstance(output_explanation, list) else [output_explanation]
            )
        return ToolCallValidationResult(
            is_completed=True,
            expected_tool_call=expected_tool_call,
    ...
    def _aggregate_failure(
        ...
        """
        tool_call_name = expected_tool_call.tool
        general_explnation_str = (
            f'Tool "{tool_call_name}" call has failed input arguments check or output check.'
        )
    ...
    @staticmethod
    async def validate_tool_call_parameter_by_llmj(
        llm_client,
        param_name: str,
        param_check: str,
        actual_tool_call: Step,
        num_judge_trials: int,
        include_judge_explanation: bool,
    ) -> tuple[bool, List[str]]:
        input_arg_value: Any = ""
        if actual_tool_call.tool_input_args is not None:
        ...
        (
            ...
            judge_explanations,
        ) = await Validator.get_majority_voted_score_from_judge_responses_optimised(
            llm_client=llm_client, prompt=prompt, no_of_trials=num_judge_trials
        )
        is_completed = True if majority_voted_score == 1 else False
        explanations = []
        if is_completed:
            explanations.append(ARGUMENT_PASSED_LLMJ_EXPLANATION.format(param_name=param_name))
        else:
            explanations.append(ARGUMENT_FAILED_LLMJ_EXPLANATION.format(param_name=param_name))
    ...
    @staticmethod
    def construct_tool_correctness_llmj_prompt(
        arg_value: str,
        gt_param_check: str,
        actual_tool_call: Step,
        is_input_arg: bool = True,
    ) -> str:
        if is_input_arg:
            ...
            if actual_tool_call.tool_input_args is not None:
                for arg in actual_tool_call.tool_input_args:
                    agent_tool_call_step_str += (
                        f"ToolInputParameter(name='{arg.name}', value='{arg.value}'),\n"
                    )
            agent_tool_call_step_str += "])"
        else:
    ...
    @staticmethod
    def validate_tool_call_parameter_by_exact_match(
        expected_tool_param: ToolInputParameter, actual_tool_call_step: Step
    ) -> tuple[bool, str]:
        param_name = expected_tool_param.name
        if actual_tool_call_step.tool_input_args is not None:
            for input_arg in actual_tool_call_step.tool_input_args:
                if input_arg.name.strip() == param_name.strip():
                    if input_arg.value == expected_tool_param.value:
                        return True, ARGUMENT_PASSED_EXACT_MATCH_EXPLANATION.format(
                            param_name=param_name
                        )
                    else:
                        return False, ARGUMENT_FAILED_EXACT_MATCH_EXPLANATION.format(
                            param_name=param_name,
                            expected_value=expected_tool_param.value,
                            expected_type=type(expected_tool_param.value),
                            actual_value=input_arg.value,
                            actual_type=type(input_arg.value),
                        )
        return False, ARGUMENT_NOT_FOUND_EXPLANATION.format(
            param_name=param_name,
            expected_value=expected_tool_param.value,
            expected_type=type(expected_tool_param.value),
        )
    ...
    @staticmethod
    def validate_tool_call_output_by_exact_match(
        expected_output_value: Any, actual_tool_call_step: Step
    ) -> tuple[bool, str]:
        actual_output = actual_tool_call_step.tool_output
        if actual_output == expected_output_value:
            return True, OUTPUT_PASSED_EXACT_MATCH_EXPLANATION
        else:
            return False, OUTPUT_FAILED_EXACT_MATCH_EXPLANATION.format(
                expected_output=expected_output_value,
                expected_type=type(expected_output_value),
                actual_output=actual_output,
                actual_type=type(actual_output),
            )

    @staticmethod
    async def validate_tool_call_output_by_llmj(
        llm_client,
        output_check: str,
        actual_tool_call: Step,
        num_judge_trials: int,
        ...
        # In cases where the tool call does not return any output, we set it to an empty string
        prompt = ToolCallCompletionValidator.construct_tool_correctness_llmj_prompt(
            arg_value=actual_tool_call.tool_output
            if actual_tool_call.tool_output is not None
            else "",
            gt_param_check=output_check,
            actual_tool_call=actual_tool_call,
            is_input_arg=False,
        )
        (
        ...
        explanations = []
        if is_completed:
            explanations.append(OUTPUT_PASSED_LLMJ_EXPLANATION)
        else:
            explanations.append(OUTPUT_FAILED_LLMJ_EXPLANATION)
